...
describe('LiveboardDirect', async () => {
  ...
  test('direct-load-liveboard', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-liveboard', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['IRAIL_TEST_LIVEBOARD_ENTID']
