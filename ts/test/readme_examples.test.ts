...
//     (\`new IrailSDK(...)\` / \`IrailSDK.test(...)\` become
//     \`IrailSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"composition":{"test01":{"id":"test01"}},"connection":{"test01":{"id":"test01"}},"disturbance":{"test01":{"id":"test01"}},"liveboard":{"test01":{"id":"test01"}},"log":{"test01":{"id":"test01"}},"occupancy":{"test01":{"id":"test01"}},"station":{"test01":{"id":"test01"}},"vehicle":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new IrailSDK(...)\` and any
// \`IrailSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+IrailSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/IrailSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
