...
boru-driven command-line client **and** interactive REPL for the Irail
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/irail-cli)
make build
...
./irail-cli load 1 composition            # {id:1} shorthand
./irail-cli load '{id:1}' composition       # explicit match map

# 5. Override the API base URL for a single call
IRAIL_BASE=https://api.example.com ./irail-cli load 1 composition

# 6. No arguments -> interactive REPL
./irail-cli
irail> load 1 composition
irail> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/irail-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/irail-cli load 1 composition
   ```
...
```sh
./irail-cli load 1 composition          # scalar shorthand for {id:1}
./irail-cli load '{id:1}' composition     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `irail>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `composition`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/irail-cli`. |
...
(`./irail-cli <expr>`) and interactively (the REPL), and expressions compose the
