...
The Lua SDK for the Irail API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Composition(nil):load({ id = "example_id" }, nil)
...
local result, err = client:Irail(nil):load(
  { id = "test01" }, nil
)
...
The main module (`irail_sdk`) exports the SDK constructor
