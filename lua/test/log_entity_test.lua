...
describe("LogEntity", function()
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:Log(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:Log(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set IRAIL_TEST_LOG_ENTID JSON to run live")
      return
    end
    ...
    local log_ref01_match = {}

    local log_ref01_list_result, err = log_ref01_ent:list(log_ref01_match, nil)
    assert.is_nil(err)
    assert.is_table(log_ref01_list_result)
...
function log_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("IRAIL_TEST_LOG_ENTID")
