...
local OccupancyEntity = {}
OccupancyEntity.__index = OccupancyEntity


function OccupancyEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, OccupancyEntity)
  self._name = "occupancy"
  self._client = client
...
function OccupancyEntity:get_name()
  return self._name
end


function OccupancyEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return OccupancyEntity.new(self._client, opts)
end
...
function OccupancyEntity:mark_deleted()
  self._deleted = true
end


function OccupancyEntity:deleted()
  return true == self._deleted
end


function OccupancyEntity:data_set(args)
  if args ~= nil then
...
function OccupancyEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function OccupancyEntity:match_set(args)
  if args ~= nil then
...
function OccupancyEntity:match_get()
...
function OccupancyEntity:stream(action, args, callopts)
...
function OccupancyEntity:_run_op(ctx, post_done)
  local utility = self._utility
