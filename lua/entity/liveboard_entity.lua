...
local LiveboardEntity = {}
LiveboardEntity.__index = LiveboardEntity


function LiveboardEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, LiveboardEntity)
  self._name = "liveboard"
  self._client = client
...
function LiveboardEntity:get_name()
  return self._name
end


function LiveboardEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return LiveboardEntity.new(self._client, opts)
end
...
function LiveboardEntity:mark_deleted()
  self._deleted = true
end


function LiveboardEntity:deleted()
  return true == self._deleted
end


function LiveboardEntity:data_set(args)
  if args ~= nil then
...
function LiveboardEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function LiveboardEntity:match_set(args)
  if args ~= nil then
...
function LiveboardEntity:match_get()
...
function LiveboardEntity:stream(action, args, callopts)
...
---@param reqmatch LiveboardLoadMatch
---@param ctrl? table
---@return Liveboard
---@return string? err
function LiveboardEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function LiveboardEntity:_run_op(ctx, post_done)
  local utility = self._utility
