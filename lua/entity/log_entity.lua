...
local LogEntity = {}
LogEntity.__index = LogEntity


function LogEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, LogEntity)
  self._name = "log"
  self._client = client
...
function LogEntity:get_name()
  return self._name
end


function LogEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return LogEntity.new(self._client, opts)
end
...
function LogEntity:mark_deleted()
  self._deleted = true
end


function LogEntity:deleted()
  return true == self._deleted
end


function LogEntity:data_set(args)
  if args ~= nil then
...
function LogEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function LogEntity:match_set(args)
  if args ~= nil then
...
function LogEntity:stream(action, args, callopts)
...
---@param reqmatch LogListMatch
---@param ctrl? table
---@return Log[]
---@return string? err
function LogEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function LogEntity:_run_op(ctx, post_done)
  local utility = self._utility
