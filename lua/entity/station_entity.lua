-- Irail SDK Station entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local StationEntity = {}
StationEntity.__index = StationEntity


function StationEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, StationEntity)
  self._name = "station"
  self._client = client
...
function StationEntity:get_name()
  return self._name
end


function StationEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return StationEntity.new(self._client, opts)
end
...
function StationEntity:mark_deleted()
  self._deleted = true
end


function StationEntity:deleted()
  return true == self._deleted
end


function StationEntity:data_set(args)
  if args ~= nil then
...
function StationEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function StationEntity:match_set(args)
  if args ~= nil then
...
function StationEntity:match_get()
...
function StationEntity:stream(action, args, callopts)
...
---@param reqmatch StationLoadMatch
---@param ctrl? table
---@return Station
---@return string? err
function StationEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function StationEntity:_run_op(ctx, post_done)
  local utility = self._utility
