...
local DisturbanceEntity = {}
DisturbanceEntity.__index = DisturbanceEntity


function DisturbanceEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DisturbanceEntity)
  self._name = "disturbance"
  self._client = client
...
function DisturbanceEntity:get_name()
  return self._name
end


function DisturbanceEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DisturbanceEntity.new(self._client, opts)
end
...
function DisturbanceEntity:mark_deleted()
  self._deleted = true
end


function DisturbanceEntity:deleted()
  return true == self._deleted
end


function DisturbanceEntity:data_set(args)
  if args ~= nil then
...
function DisturbanceEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DisturbanceEntity:match_set(args)
  if args ~= nil then
...
function DisturbanceEntity:match_get()
...
function DisturbanceEntity:stream(action, args, callopts)
...
function DisturbanceEntity:_run_op(ctx, post_done)
  local utility = self._utility
