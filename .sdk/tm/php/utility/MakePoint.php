...
// Irail SDK utility: make_point

require_once __DIR__ . '/../core/Helpers.php';

class IrailMakePoint
{
    public static function call(IrailContext $ctx): array
    {
        if (isset($ctx->out['point'])) {
            ...
            if ($ctx->out['point'] instanceof IrailError) {
        ...
        if (count($op->points) === 1) {
            ...
            foreach ($op->points as $p) {
                $point = $p;
                $select_def = IrailHelpers::to_map(\Voxgig\Struct\Struct::getprop($p, 'select'));
                $found = true;
            ...
            if ($reqselector) {
                $req_action = \Voxgig\Struct\Struct::getprop($reqselector, '$action');
                if ($req_action && $point) {
                    $point_select = IrailHelpers::to_map(\Voxgig\Struct\Struct::getprop($point, 'select'));
