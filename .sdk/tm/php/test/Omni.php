...
//    only - a typed IrailContext there would make every ctx
...
final class IrailOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new IrailContext($clean, null);
    ...
    public static function err_from_map($m): ?IrailError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new IrailError(is_string($code) ? $code : '', $msg);
    }
