...
{
    private static function utility(): IrailUtility
    {
        return new IrailUtility();
    }
    ...
    private static function ctx(array $over = []): IrailContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new IrailContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new IrailOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof IrailError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new IrailOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = IrailMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new IrailOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = IrailMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new IrailOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = IrailMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new IrailSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = IrailMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new IrailResponse([]),
            'result' => new IrailResult([]),
        ]);
        [, $err] = IrailMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new IrailSpec(['step' => 's']),
            'response' => new IrailResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new IrailResult([]),
        ]);
        [, $err] = IrailMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new IrailSpec(['step' => 's']),
            'response' => new IrailResponse(self::resp(200, ['v' => 1])),
            'result' => new IrailResult([]),
        ]);
        [, $err] = IrailMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new IrailSpec(['step' => 's']),
            'response' => new IrailResponse(self::resp(200, ['v' => 2])),
            'result' => new IrailResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        IrailMakeResponse::call($ctx);
    ...
    {
        $preset = new IrailResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new IrailSpec([]),
            'response' => new IrailResponse([]),
            'result' => new IrailResult([]),
        ...
        [$got, $err] = IrailMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new IrailResult([])]);
        [, $err] = IrailMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new IrailSpec([]), 'result' => null]);
        [, $err] = IrailMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new IrailSpec(['step' => 's']),
            'result' => new IrailResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new IrailOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = IrailMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new IrailSpec(['step' => 's']),
            'result' => new IrailResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new IrailOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = IrailMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new IrailResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new IrailSpec([]),
            'result' => new IrailResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = IrailMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $utility = self::utility();
        $boom = new IrailError('boom', 'boom');
        $utility->fetcher = function (IrailContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new IrailSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = IrailMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (IrailContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = IrailMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(IrailResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (IrailContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new IrailSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = IrailMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new IrailResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new IrailSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = IrailMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = IrailMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = IrailMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new IrailResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, IrailDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new IrailResult(['ok' => false])]);
        $this->expectException(IrailError::class);
        IrailDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new IrailResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', IrailMakeError::call($ctx, null));
    }
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new IrailBaseFeature();
        $b = new IrailBaseFeature();
        IrailFeatureAdd::call($ctx, $a);
        IrailFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): IrailBaseFeature
    {
        $f = new IrailBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    private static function auth_ctx(array $options, ?IrailSpec $spec): IrailContext
    {
    ...
    private static function auth_bags(): IrailSpec
    {
        return new IrailSpec(['headers' => [], 'query' => []]);
    }
    ...
    {
        $ctx = self::ctx([
            'response' => new IrailResponse(['headers' => null]),
            'result' => new IrailResult([]),
        ]);
        IrailResultHeaders::call($ctx);
