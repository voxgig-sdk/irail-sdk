...
from irail_sdk.irail_types import (
    Composition,
...
class CompositionEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return CompositionEntity(self._client, opts)
    ...
    def match_get(self) -> Composition:
    ...
    def load(self, reqmatch=None, ctrl=None) -> Composition:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.Composition().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
