...
class IrailTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            IrailTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = IrailTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = IrailTestRunner.getenv("IRAIL_TEST_LIVE")
        override = IrailTestRunner.getenv("IRAIL_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = IrailTestRunner.getenv(key)
        ...
        explain = IrailTestRunner.getenv("IRAIL_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["IRAIL_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if IrailTestRunner._test_control is not None:
            return IrailTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                IrailTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            IrailTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = IrailTestRunner.load_test_control()
...
def load_env_local():
    IrailTestRunner.load_env_local()


def env_override(m):
    return IrailTestRunner.env_override(m)


def entity_data(v):
    return IrailTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return IrailTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return IrailTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return IrailTestRunner.load_test_control()


def live_client_options():
    return IrailTestRunner.live_client_options()


def live_delay_ms():
    return IrailTestRunner.live_delay_ms()
