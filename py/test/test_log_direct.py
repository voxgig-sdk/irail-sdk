...
class TestLogDirect:
    ...
    def test_should_direct_list_log(self):
        setup = _log_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-log", "live" if setup["live"] else "unit")
        if _skip:
