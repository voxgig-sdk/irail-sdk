...
The Python SDK for the Irail API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Composition(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.Irail(None).load(
    {"id": "test01"}, None
)
