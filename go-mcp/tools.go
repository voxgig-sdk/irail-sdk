...
type Args struct {
	Entity string         `json:"entity" jsonschema:"composition | connection | disturbance | liveboard | log | occupancy | station | vehicle"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.IrailSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "irail_list",
		Description: "List records from Irail. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "irail_load",
		Description: "Load a single record from Irail. " +
...
func runOp(client *sdk.IrailSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.IrailSDK, name string) (sdk.IrailEntity, error) {
	switch strings.ToLower(name) {
	case "composition":
		return client.Composition(nil), nil
	case "connection":
		return client.Connection(nil), nil
	case "disturbance":
		return client.Disturbance(nil), nil
	case "liveboard":
		return client.Liveboard(nil), nil
	case "log":
		return client.Log(nil), nil
	case "occupancy":
		return client.Occupancy(nil), nil
	case "station":
		return client.Station(nil), nil
	case "vehicle":
