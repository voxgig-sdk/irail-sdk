...
[MCP](https://modelcontextprotocol.io) server exposing the Irail SDK as
two agent tools — `irail_list` and `irail_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/irail-mcp)
make build

# 2. Provide credentials via the environment
export IRAIL_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user irail \
  -- /absolute/path/to/irail-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./irail-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// irail_list: first page of records
{ "entity": "connection" }
{ "entity": "connection", "query": { } }

// irail_load: one record by id
{ "entity": "composition", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/irail-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user irail \
     -- "$PWD"/dist/*/irail-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `irail_list` and `irail_load` tools now appear
   in new sessions. Ask the agent to *"list connection using irail"*
   and it calls `irail_list` with `{"entity":"connection"}`.
...
```sh
export IRAIL_APIKEY=sk_live_xxx            # API key
export IRAIL_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `irail_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `irail_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `IRAIL_APIKEY` | API key sent with every request. |
| `IRAIL_BASE` | Optional override of the API base URL. |

### Entities

The 8 entities valid as the `entity` argument:

composition | connection | disturbance | liveboard | log | occupancy | station | vehicle

### Smoke test via HTTP (raw JSON-RPC)

```sh
./irail-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"irail_load","arguments":{"entity":"composition","query":{"id":1}}}}'
```
