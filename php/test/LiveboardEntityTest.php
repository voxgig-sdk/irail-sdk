...
class LiveboardEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = IrailSDK::test(null, null);
        $ent = $testsdk->Liveboard(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = liveboard_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "liveboard." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set IRAIL_TEST_LIVEBOARD_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $liveboard_ref01_data = null;
        if (count($liveboard_ref01_data_raw) > 0) {
...
function liveboard_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/liveboard/LiveboardTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["liveboard01", "liveboard02", "liveboard03"] as $k) {
    ...
    $entid_env_raw = getenv("IRAIL_TEST_LIVEBOARD_ENTID");
