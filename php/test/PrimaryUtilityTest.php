...
// Irail SDK primary utility test
//
...
//   IrailOmni::unwrap, which throws the err so omni can match it
...
//   builds the typed context with IrailOmni::ctx_from_map, runs the
//   utility, and writes the observable state back with
//   IrailOmni::sync_ctx BEFORE throwing any error - that map is
...
class PrimaryUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = IrailOmni::makeRunner(
                self::TEST_JSON_FILE, IrailSDK::test(null, null));
    ...
    private static function client(): IrailSDK
    {
    ...
    private static function auth_credential(
        ...
        $ctx->spec = new IrailSpec(['headers' => [], 'query' => []]);
    ...
    public function test_done_basic(): void
    {
        ...
        $this->runsection('done', function (array $ctxmap) use ($client, $utility) {
            $ctx = IrailOmni::ctx_from_map($ctxmap, $client, $utility);
    ...
    public function test_make_error_basic(): void
    {
        ...
        $this->runsection('makeError', function ($ctxmap = null, $errmap = null) use ($client, $utility) {
            $ctx = IrailOmni::ctx_from_map(
                is_array($ctxmap) ? $ctxmap : [], $client, $utility);
            $err = IrailOmni::err_from_map(is_array($errmap) ? $errmap : null);
    ...
    {
        $hook_client = IrailSDK::test(null, null);
        $hook_utility = $hook_client->get_utility();
        $ctx = self::make_test_ctx($hook_client, $hook_utility);

        $called = false;
        $hook_feature = new class extends IrailBaseFeature {
            public $hook_fn = null;
            public function TestHook(IrailContext $ctx): void
            {
    ...
    {
        $init_client = IrailSDK::test(null, null);
        ...
        $init_called = false;
        $feature = new class extends IrailBaseFeature {
            public $init_fn = null;
    ...
    public function test_make_result_no_result(): void
    {
        ...
        $utility = $client->get_utility();
        $ctx = self::make_test_full_ctx($client, $utility);
        $ctx->spec = new IrailSpec(['step' => 'start']);
        $ctx->result = null;
    ...
    public function test_make_url_basic(): void
    {
        ...
        $this->runsection('makeUrl', function (array $ctxmap) use ($client, $utility) {
            ...
            return IrailOmni::unwrap(($utility->make_url)($ctx));
        });
    }
    ...
    public function test_prepare_auth_basic(): void
    {
        ...
        $auth_client = IrailSDK::test(null, is_array($setup) ? $setup : null);
        ...
        $this->runsection('prepareAuth', function (array $ctxmap) use ($auth_client, $auth_utility, $utility) {
            $ctx = IrailOmni::ctx_from_map($ctxmap, $auth_client, $auth_utility);
    ...
    public function test_result_basic_basic(): void
    {
        ...
        $this->runsection('resultBasic', function (array $ctxmap) use ($client, $utility) {
            ...
            if ($result->err !== null) {
                $err_msg = ($result->err instanceof IrailError)
