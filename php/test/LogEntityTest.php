...
class LogEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Log(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Log(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = log_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "log." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set IRAIL_TEST_LOG_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function log_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/log/LogTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["log01", "log02", "log03"] as $k) {
    ...
    $entid_env_raw = getenv("IRAIL_TEST_LOG_ENTID");
