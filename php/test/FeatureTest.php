...
class FtCtrl extends IrailControl
{
...
class FtClient
{
    ...
    private IrailUtility $utility;

    public function __construct(IrailUtility $utility, array $options)
    {
    ...
    public function get_utility(): IrailUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (IrailContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (IrailContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = IrailConfig::make_feature($name);
    ...
    private static function buildUrl(IrailSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof IrailError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new IrailResponse($fetched);
        }
    ...
    private function fail(IrailContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (IrailContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
