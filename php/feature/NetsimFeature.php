...
class IrailNetsimFeature extends IrailBaseFeature
{
    ...
    private function _simulate(IrailContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(IrailContext $ctx, array $applied): void
    {
        $client = $this->client;
