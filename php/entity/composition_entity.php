...
class CompositionEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new CompositionEntity($this->_client, $opts);
    }

    /**
     * @param Composition|array $args Composition data (assoc-array) to store.
     */
    ...
    /**
     * @return Composition|array The current Composition data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Composition fields).
     */
    ...
    /**
     * @return array The current match filter (any subset of Composition fields).
     */
     ...
     *
     * @param CompositionLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed CompositionLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Composition|array The loaded Composition as an assoc-array at the
     *   SDK boundary; throws IrailError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = IrailHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
