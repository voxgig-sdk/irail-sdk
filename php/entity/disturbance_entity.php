...
class DisturbanceEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new DisturbanceEntity($this->_client, $opts);
    }

    /**
     * @param Disturbance|array $args Disturbance data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = IrailHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Disturbance|array The current Disturbance data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Disturbance fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = IrailHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Disturbance fields).
     */
    ...
    /**
     * List Disturbance items matching the given filter.
     *
     * @param DisturbanceListMatch|array|null $reqmatch Match filter (any subset
     *   of Disturbance fields) as an assoc-array; DisturbanceListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Disturbance[]|array A list of Disturbance items as assoc-arrays at
     *   the SDK boundary; throws IrailError on failure (item-5 convention).
     */
