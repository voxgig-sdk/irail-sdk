...
class DisturbanceDirectTest < Minitest::Test
  def test_direct_list_disturbance
    setup = disturbance_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-disturbance", setup[:live] ? "live" : "unit")
    if _should_skip
...
def disturbance_direct_setup(mockres)
