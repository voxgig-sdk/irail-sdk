...
class CompositionEntity
  ...
  def make
    opts = @_entopts.dup
    CompositionEntity.new(@_client, opts)
  end
  ...
  # @return [Composition, Hash] the current Composition data
  def data_get
  ...
  # @return [Hash] the current match filter (any subset of Composition fields)
  def match_get
  ...
  #
  # @param reqmatch [CompositionLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Composition.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Composition, Hash] the loaded Composition; raises IrailError on failure
