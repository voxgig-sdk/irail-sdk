...
class DisturbanceEntity
  ...
  def make
    opts = @_entopts.dup
    DisturbanceEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = IrailHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Disturbance, Hash] the current Disturbance data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = IrailHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Disturbance fields)
  def match_get
  ...
  #
  # @param reqmatch [DisturbanceListMatch, Hash, nil] match filter (any subset of
  #   Disturbance fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Disturbance>, Array] the matching Disturbance items; raises IrailError on failure
