...
class LiveboardEntity
  ...
  def make
    opts = @_entopts.dup
    LiveboardEntity.new(@_client, opts)
  end
  ...
  # @return [Liveboard, Hash] the current Liveboard data
  def data_get
  ...
  # @return [Hash] the current match filter (any subset of Liveboard fields)
  def match_get
  ...
  #
  # @param reqmatch [LiveboardLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Liveboard.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Liveboard, Hash] the loaded Liveboard; raises IrailError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = IrailHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
