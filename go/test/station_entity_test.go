...
func TestStationEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set IRAIL_TEST_STATION_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		stationRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.station")))
		...
		if stationRef01DataDt0Loaded == nil {
...
func stationBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("IRAIL_TEST_STATION_ENTID")
	...
	idmapResolved := core.ToMapAny(env["IRAIL_TEST_STATION_ENTID"])
