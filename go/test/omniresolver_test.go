...
import (
	...
	omni "github.com/voxgig-sdk/irail-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.IrailError); is {
				out := map[string]any{
					"name":    "IrailError",
...
func omniCtx(arg any, client *sdk.IrailSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.IrailSDK); is {
			client = live
