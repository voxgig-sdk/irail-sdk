...
func TestConnectionEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set IRAIL_TEST_CONNECTION_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func connectionBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("IRAIL_TEST_CONNECTION_ENTID")
	...
	idmapResolved := core.ToMapAny(env["IRAIL_TEST_CONNECTION_ENTID"])
